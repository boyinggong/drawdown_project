...
Figure \ref{fig: risk_meausre_RMZ} shows the rolling VaR, volatility, ES, and maximum drawdown with rolling six-month periods. Under Gaussian distribution, VaR, ES, and volatility is linearly dependent. For empirical data where the return distribution has fat tails and distinct kurtosis, they still strongly correlated. (With average correlation $>$ 95\% for 6 month rolling window) 

\begin{figure}
\centering 
\includegraphics[width = 1\textwidth]{../results/risk_measure_RMZ}
\caption{Comparison of different risk measures of US equity REITs index (RMZ)} 
\label{fig: risk_meausre_RMZ}
\end{figure}



CED is also closely related with other risk measures. However, the correlation between CED and the other three risk measures are slightly weaker than the correlation among volatility, VaR and ES. Table \ref{table:corrRiskMeasureCED} shows the correlation between CED and other risk measures calculated based on six-month rolling risk measures for every asset. 
